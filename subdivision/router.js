...
async function fetchVertexElevation(vertexIndices, runId) {
  ...
  console.groupCollapsed('📨 Elevation request batch');
  console.log('Relay', settings.nknRelay);
  console.log('Request payload', req);
  console.log('Vertices', requests.map(r => ({ idx: r.idx, lat: r.lat, lon: r.lon, geohash: r.geohash })));
  console.groupEnd();
  ...
  try {
    ...
    console.log('📥 Raw elevation response', resp);
    if (runId !== currentRegenerationRunId || cancelRegeneration) {
      resetPending();
      return;
    }
